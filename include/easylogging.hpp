...
#if ((!ELPP_MINGW && !ELPP_COMPILER_CLANG) ||                                  \
...
public:
  ...
  timevalToString(struct timeval tval, const char *format,
    ...
    parseFormat(buff_, kBuffSize, format, &timeInfo,
                static_cast<std::size_t>(tval.tv_usec / msWidth->m_offset),
                msWidth);
...
public:
  ...
  static inline bool installCallback(const std::string &id,
  ...
  template <typename T, typename TPtr>
  static inline void uninstallCallback(const std::string &id,
                                       std::map<std::string, TPtr> *mapT) {
    if (mapT->find(id) != mapT->end()) {
      mapT->erase(id);
    }
  ...
  template <typename T, typename TPtr>
  static inline T *callback(const std::string &id,
                            std::map<std::string, TPtr> *mapT) {
      ...
      while ((foundAt = formatCopy.find(specifier, foundAt + 1)) !=
        ...
        if (foundAt > 0 &&
          ...
          if (hasFlag(flag)) {
            ...
            // is turned to '%' even after specifier resolution - this is
            // because we only replaceFirst specifier
...
public:
  ...
  /// Also note that configuration types ConfigurationType::MillisecondsWidth
  ...
  void build(Configurations *configurations) {
    ...
    for (Configurations::const_iterator it = configurations->begin();
      ...
      if (conf->configurationType() == ConfigurationType::Enabled) {
        ...
        // performance sensative operation, we can live with another loop; (by
        // the way this loop is not very heavy either)
...
private:
  ...
  base::LogStreamsReferenceMap m_logStreamsReference;
  std::map<std::string, base::type::LoggerRegistrationCallbackPtr>
      m_loggerRegistrationCallbacks;
  friend class el::base::Storage;
...
public:
  ...
  explicit Storage(const LogBuilderPtr &defaultLogBuilder)
      :
        ...
        m_flags(0x0), m_vRegistry(new base::VRegistry(0, &m_flags)),
#if ELPP_ASYNC_LOGGING
...
public:
  PerformanceTracker(
      ...
      : m_blockName(blockName), m_timestampUnit(timestampUnit),
        ...
        m_hasChecked(false), m_lastCheckpointId(std::string()),
        m_enabled(false) {
...
public:
  ...
  /// using identity
  static inline Logger *
  reconfigureLogger(const std::string &identity,
...
public:
  ...
  static inline const std::string version(void) { return std::string("9.89"); }
