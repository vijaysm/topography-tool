 ...
 * NetcdfMeshIO::load_point_cloud_from_file(mb, mesh_set, "input.nc", options,
 * entities);
 *
 * // Write scalar fields
 * NetcdfWriteRequest request;
 * request.scalar_var_names = {"temperature", "pressure"};
 * NetcdfMeshIO::write_point_scalars_to_file(mb, "template.nc", "output.nc",
 * request, entities);
 * @endcode
 */
class NetcdfMeshIO {
public:
  static constexpr size_t MAX_ELEMENTS_PER_CHUNK =
      250 * 1000 * 1000; // 250M elements, well below INT_MAX
   ...
   */
  static ErrorCode
  load_point_cloud_from_file(Interface *mb, EntityHandle mesh_set,
   ...
   * variables with automatic type conversion and chunked I/O for large
   * datasets.
   *
   * Algorithm:
   ...
   */
  static ErrorCode
  write_point_scalars_to_file(Interface *mb, const std::string &template_file,
