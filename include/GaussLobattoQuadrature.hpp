...
public:
  ...
  inline static moab::ErrorCode GetPoints(int nCount, std::vector<double> &dG,
                                          ...
                                          double dXi0 = 0.0, double dXi1 = 1.0);
};

moab::ErrorCode GaussLobattoQuadrature::GetPoints(int nCount,
  ...
  if (nCount < 2 || nCount > 10) {
    MB_CHK_SET_ERR(moab::MB_INVALID_SIZE,
                   "Invalid count (" << nCount
                                     << "): Minimum count 2, maximum count 10");
  }
  ...
  default:
    MB_CHK_SET_ERR(moab::MB_UNHANDLED_OPTION,
                   "Unsupported quadrature order ("
                       << nCount << "): Maximum supported is 10");
  }
