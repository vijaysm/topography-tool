...
#pragma omp critical
            LOG(INFO) << "Processing element " + std::to_string(elem_idx) +
                             " of " + std::to_string(point_data.size());
        }
...
void ScalarRemapper::print_remapping_statistics() {
    ...
    for (const auto &field_pair : m_mesh_data.d_scalar_fields) {
        ...
        double min_val =
            *std::min_element(field_data.begin(), field_data.end());
        double max_val =
            *std::max_element(field_data.begin(), field_data.end());
...
static ErrorCode write_to_tag(moab::Interface *interface,
    ...
    if (sizeof(T) == sizeof(double)) {
        MB_CHK_ERR(interface->tag_get_handle(tag_name.c_str(), 1,
                                             MB_TYPE_DOUBLE, tag,
...
ErrorCode ScalarRemapper::write_to_tags(const std::string &tag_prefix) {
    ...
    for (const auto &field_pair : m_mesh_data.d_scalar_fields) {
        ...
        MB_CHK_ERR(write_to_tag<double>(m_interface, tag_name,
                                        m_mesh_data.elements, field_data));

        LOG(INFO) << "Created tag '" << tag_name << "' with "
                  << field_data.size() << " values";
    }
    ...
    for (const auto &field_pair : m_mesh_data.i_scalar_fields) {
        ...
        std::string tag_name = tag_prefix + var_name;
        MB_CHK_ERR(write_to_tag<int>(m_interface, tag_name,
                                     m_mesh_data.elements, field_data));
...
ErrorCode ScalarRemapper::build_grid_locator(
    ...
    try {
        ...
        if (!point_data.latitudes.empty() && !point_data.longitudes.empty()) {
            LOG(INFO) << "Using stored 1D arrays: "
                      << point_data.latitudes.size() << " latitudes x "
            ...
            LOG(INFO) << "RegularGridLocator build time: "
                      << build_duration.count() << " ms";
        } else {
...
#pragma omp parallel
    ...
    try {
        ...
        LOG(INFO) << "Built KD-tree index for " << point_data.size()
                  << " points";
...
ScalarRemapper::kdtree_search(const PointType3D &target_point,
    ...
    try {
        ...
        if (search_radius > 0.0) {
            ...
            m_kdtree->knnSearch(target_point.data(), max_neighbors,
...
Remapper_ApplyLocalMap(Interface * /* mb */, const double nodes[12],
    ...
    if (dDx1G || dDx2G) {
        ...
        if (dDx1G) {
            *dDx1G =
                dDenomTerm * CartVect(-dXc * (dYc * dDx1F[1] + dZc * dDx1F[2]) +
        ...
        if (dDx2G) {
            *dDx2G =
                dDenomTerm * CartVect(-dXc * (dYc * dDx2F[1] + dZc * dDx2F[2]) +
...
double Remapper_GenerateFEMetaData(Interface *mb, EntityHandle face,
    ...
    int nConnectivity;
    MB_CHK_SET_ERR(
        mb->get_connectivity(face, connectivity, nConnectivity, true),
    ...
    for (int j = 0; j < nP; j++) {
        for (int i = 0; i < nP; i++) {
            ...
            Remapper_ApplyLocalMap(mb, nConnCoords, dG[i], dG[j], nodeGLL,
                                   &dDx1G, &dDx2G);
...
ErrorCode PCDiskAveragedProjectionRemapper::perform_remapping(
    ...
    if (m_target_is_spectral) {
        LOG(INFO) << "Starting PC averaged spectral projection with "
                  << point_data.size()
                  << " point cloud points and spectral order "
                  << spectral_order;
...
#pragma omp critical
        ...
        const EntityHandle *connectivity;
        int nConnectivity;
        rval = m_interface->get_connectivity(face, connectivity, nConnectivity,
                                             true);
        if (MB_SUCCESS != rval) {
            element_errors[elem_idx] = MB_FAILURE;
            continue;
        ...
        /* double dNumericalArea = */ Remapper_GenerateFEMetaData(
            m_interface, face, dG, dW, dataGLLJacobian,
        ...
        for (int j = 0; j < nP; j++) {
            for (int i = 0; i < nP; i++) {
                int gll_idx = j * nP + i;
                CartVect nodeGLL;
                rval = Remapper_ApplyLocalMap(m_interface, nConnCoords, dG[i],
                                              dG[j], nodeGLL);
                if (MB_SUCCESS != rval) {
                ...
                auto nearest_points = find_nearest_point(
                    gll_point, point_data, nullptr, &search_radius);
                ...
                if (!has_valid_results) {
                    ...
                    const CoordinateType no_radius =
                        0.0; // 0.0 means no radius constraint
                    nearest_points = find_nearest_point(
                        gll_point, point_data, &single_neighbor, &no_radius);
                }
...
#pragma omp critical
                        LOG(WARNING)
                            << "WARNING: Element " << elem_idx << ", GLL node ("
                            << i << "," << j
                            << ") has ZERO neighbors even after fallback!";
                    }
                ...
                for (const auto &var_name : m_config.scalar_var_names) {
                    ...
                    if (!m_config.is_usgs_format &&
                        ...
                        auto ivar_it =
                            point_data.i_scalar_variables.find(var_name);
                        if (ivar_it != point_data.i_scalar_variables.end()) {
                            const auto &values = ivar_it->second;
                            ...
                            for (size_t k = 0; k < nearest_points.size(); ++k) {
                                ...
                                if (pt_idx < values.size()) {
                                    weighted_sum +=
                                        inv_weights[k] *
                                        static_cast<double>(values[pt_idx]);
                                    weight_sum += inv_weights[k];
                                }
                            }
                        }
                    ...
                    if (fabs(weight_sum) > 0.0) {
                        ...
                        total_weights[var_name] +=
                            gll_weight; // Track weight per variable
                    }
                }
            }
        }
        ...
        for (const auto &var_name : m_config.scalar_var_names) {
            ...
            if (it != element_averages_d.end() &&
                wt_it != total_weights.end() && fabs(wt_it->second) > 0.0) {
...
#pragma omp critical
            LOG(INFO) << "Processing element " + std::to_string(elem_idx) +
                             " of " + std::to_string(elements.size());
        }
        ...
        const double search_radius =
            std::sqrt(vertex_areas[elem_idx] / 4.0 /
                      M_PI); // convert from radians^2 to degrees^2
