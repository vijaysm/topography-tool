...
ScalarRemapper::ScalarRemapper( Interface* interface, EntityHandle mesh_set )
    : m_interface( interface ), m_mesh_set( mesh_set ), m_target_is_spectral( false ), m_self_remapping( false ),
      m_user_search_area( 0.0 ), m_kdtree_built( false ), m_grid_locator_built( false )
{
...
ErrorCode ScalarRemapper::configure( const RemapConfig& config )
{
    ...
    else
    {
        ...
        LOG( INFO ) << "Configured scalar remapper with " << m_mesh_data.size() << " mesh elements";
    }
...
{
    LOG( INFO ) << "";
    LOG( INFO ) << "Starting scalar remapping with " << point_data.size() << " point cloud points";
...
ErrorCode ScalarRemapper::perform_self_remapping( const ParallelPointCloudReader::PointData& point_data )
{
    ...
    LOG( INFO ) << "Starting PC disk-area averaged self-projection with " << point_data.size() << " point cloud points";
...
#pragma omp parallel for schedule( dynamic, 1 ) \
    shared( m_kdtree, element_errors, point_data, m_mesh_data, m_config, m_interface, search_radius )
...
#pragma omp critical
        ...
        RLLtoXYZ_Deg( point_data.longitude( elem_idx ), point_data.latitude( elem_idx ), gll_point );
        ...
        bool has_valid_results = !nearest_points.empty() && nearest_points[0].first != static_cast< size_t >( -1 );
        ...
        if( nearest_points.empty() )
        {
            ...
            {
#pragma omp critical
                LOG( WARNING ) << "WARNING: Element " << elem_idx << ") has ZERO neighbors even after fallback!";
    ...
    if( error_count > 0 )
    {
        LOG( ERROR ) << "Warning: " << error_count << " elements failed processing out of " << point_data.size()
                     << " total elements";
    }
...
ErrorCode ScalarRemapper::extract_mesh_centroids()
{
    ...
    Range elements;
    ErrorCode rval = m_interface->get_entities_by_dimension( m_mesh_set, 2, elements );
...
void ScalarRemapper::print_remapping_statistics()
{
    ...
    for( const auto& field_pair : m_mesh_data.d_scalar_fields )
    {
        ...
        LOG( INFO ) << var_name << " - Min: " << min_val << ", Max: " << max_val << ", Avg: " << avg << " (on "
                    << field_data.size() << " elements)";
    }
                               ...
                               const std::vector< T >& data )
{
    ...
    {
        MB_CHK_ERR(
            interface->tag_get_handle( tag_name.c_str(), 1, MB_TYPE_DOUBLE, tag, MB_TAG_DENSE | MB_TAG_CREAT ) );
    }
    else
    {
        MB_CHK_ERR(
            interface->tag_get_handle( tag_name.c_str(), 1, MB_TYPE_INTEGER, tag, MB_TAG_DENSE | MB_TAG_CREAT ) );
    }

    // Set tag data on all elements
    if( !elements.empty() && !data.empty() )
    {
        MB_CHK_ERR( interface->tag_set_data( tag, elements.data(), elements.size(), data.data() ) );
    }
...
ErrorCode ScalarRemapper::build_grid_locator( const ParallelPointCloudReader::PointData& point_data )
{
    ...
    try
    {
        ...
        {
            LOG( INFO ) << "Using stored 1D arrays: " << point_data.latitudes.size() << " latitudes x "
                        << point_data.longitudes.size() << " longitudes";
            ...
            m_grid_locator =
                std::unique_ptr< RegularGridLocator >( new RegularGridLocator( lats, lons, m_config.distance_metric ) );
    ...
    for( size_t elem_idx = 0; elem_idx < m_mesh_data.centroids.size(); ++elem_idx )
    {
        ...
        if( nearest_point_idx != static_cast< size_t >( -1 ) )
        {
            ...
            for( const auto& var_name : m_config.scalar_var_names )
            {
                // Handle double precision variables
                auto var_it = point_data.d_scalar_variables.find( var_name );
                if( var_it != point_data.d_scalar_variables.end() && nearest_point_idx < var_it->second.size() )
                {
                    m_mesh_data.d_scalar_fields[var_name][elem_idx] = var_it->second[nearest_point_idx];
                    if( elem_idx < 10 )
                    {
                        LOG( INFO ) << "Double Data (" << var_name
                                    << ") at nearest neighbor element: " << nearest_point_idx << " is "
                ...
                else
                {
                    // Handle integer variables
                    auto ivar_it = point_data.i_scalar_variables.find( var_name );
                    if( ivar_it != point_data.i_scalar_variables.end() && nearest_point_idx < ivar_it->second.size() )
                    {
 ...
 */
std::vector< nanoflann::ResultItem< size_t, CoordinateType > > ScalarRemapper::find_nearest_point(
    ...
    const CoordinateType* user_search_radius )
{
    ...
    if( m_config.is_usgs_format && !m_config.use_kd_tree && m_grid_locator_built && m_grid_locator )
    {
        ...
        if( search_radius > 0.0 )
        {
            ...
            size_t num_matches = m_grid_locator->radiusSearch( query_pt, search_radius, ret_matches, false );
            if( num_matches == 0 )
            {
                ret_matches.push_back(
                    nanoflann::ResultItem< size_t, CoordinateType >( static_cast< size_t >( -1 ), 0.0 ) );
            }
        }
        else
        {
 ...
 * @return Vector of ResultItem containing point indices and distances
 */
std::vector< nanoflann::ResultItem< size_t, CoordinateType > > ScalarRemapper::linear_search_fallback(
    const PointType3D& target_point,
    const ParallelPointCloudReader::PointData& point_data,
 ...
 * @return Vector of ResultItem containing point indices and distances
 */
std::vector< nanoflann::ResultItem< size_t, CoordinateType > > ScalarRemapper::kdtree_search(
    const PointType3D& target_point,
    CoordinateType search_radius,
    ...
    try
    {
        ...
        if( search_radius > 0.0 )
        {
            ...
            const size_t num_matches =
                m_kdtree->radiusSearch( target_point.data(), search_radius, ret_matches, params );
        ...
        else
        {
            ...
            {
                ret_matches.push_back(
                    nanoflann::ResultItem< size_t, CoordinateType >( ret_index[i], out_dist_sqr[i] ) );
            }
        }
    }
    catch( const std::exception& e )
 ...
 */
std::unique_ptr< ScalarRemapper > RemapperFactory::create_remapper( RemapMethod method,
                                                                    Interface* interface,
...
static moab::ErrorCode Remapper_ApplyLocalMap( Interface* /* mb */,
                                               ...
                                               CartVect* dDx2G = nullptr )
{
    ...
    double dXc = nodes[0 * 3 + 0] * ( 1.0 - dAlpha ) * ( 1.0 - dBeta ) + nodes[1 * 3 + 0] * dAlpha * ( 1.0 - dBeta ) +
                 nodes[2 * 3 + 0] * dAlpha * dBeta + nodes[3 * 3 + 0] * ( 1.0 - dAlpha ) * dBeta;

    double dYc = nodes[0 * 3 + 1] * ( 1.0 - dAlpha ) * ( 1.0 - dBeta ) + nodes[1 * 3 + 1] * dAlpha * ( 1.0 - dBeta ) +
                 nodes[2 * 3 + 1] * dAlpha * dBeta + nodes[3 * 3 + 1] * ( 1.0 - dAlpha ) * dBeta;

    double dZc = nodes[0 * 3 + 2] * ( 1.0 - dAlpha ) * ( 1.0 - dBeta ) + nodes[1 * 3 + 2] * dAlpha * ( 1.0 - dBeta ) +
                 nodes[2 * 3 + 2] * dAlpha * dBeta + nodes[3 * 3 + 2] * ( 1.0 - dAlpha ) * dBeta;
    ...
    if( dDx1G || dDx2G )
    {
        ...
        CartVect dDx1F(
            ( 1.0 - dBeta ) * ( nodes[1 * 3 + 0] - nodes[0 * 3 + 0] ) + dBeta * ( nodes[2 * 3 + 0] - nodes[3 * 3 + 0] ),
            ( 1.0 - dBeta ) * ( nodes[1 * 3 + 1] - nodes[0 * 3 + 1] ) + dBeta * ( nodes[2 * 3 + 1] - nodes[3 * 3 + 1] ),
        ...
        {
            *dDx1G = dDenomTerm *
                     CartVect( -dXc * ( dYc * dDx1F[1] + dZc * dDx1F[2] ) + ( dYc * dYc + dZc * dZc ) * dDx1F[0],
                               -dYc * ( dXc * dDx1F[0] + dZc * dDx1F[2] ) + ( dXc * dXc + dZc * dZc ) * dDx1F[1],
                               -dZc * ( dXc * dDx1F[0] + dYc * dDx1F[1] ) + ( dXc * dXc + dYc * dYc ) * dDx1F[2] );
        }

        if( dDx2G )
        {
            *dDx2G = dDenomTerm *
                     CartVect( -dXc * ( dYc * dDx2F[1] + dZc * dDx2F[2] ) + ( dYc * dYc + dZc * dZc ) * dDx2F[0],
                               -dYc * ( dXc * dDx2F[0] + dZc * dDx2F[2] ) + ( dXc * dXc + dZc * dZc ) * dDx2F[1],
                               -dZc * ( dXc * dDx2F[0] + dYc * dDx2F[1] ) + ( dXc * dXc + dYc * dYc ) * dDx2F[2] );
        }
    }
 ...
 */
double Remapper_GenerateFEMetaData( Interface* mb,
                                    EntityHandle face,
                                    ...
                                    const bool apply_bubble_correction = false )
{
    ...
    MB_CHK_SET_ERR( mb->get_coords( connectivity, nConnectivity, nConnCoords ), "Failed to get element coordinates" );
    ...
    for( int j = 0; j < nP; j++ )
    {
        for( int i = 0; i < nP; i++ )
        {
            ...
            Remapper_ApplyLocalMap( mb, nConnCoords, dG[i], dG[j], nodeGLL, &dDx1G, &dDx2G );
            ...
            if( dJacobian <= 0.0 )
            {
                MB_CHK_SET_ERR( MB_FAILURE, "Nonpositive Jacobian detected - invalid element geometry" );
            }
...
using MOABKDTree = nanoflann::KDTreeSingleIndexAdaptor<
    nanoflann::L2_Simple_Adaptor< CoordinateType, MOABCentroidCloud, CoordinateType >,
                                            ...
                                            CoordinateType radius )
{
    ...
    const CoordinateType query_pt_sq =
        query_pt[0] * query_pt[0] + query_pt[1] * query_pt[1] + query_pt[2] * query_pt[2];
    if( query_pt_sq > 1.0 - 1e-10 && query_pt_sq < 1.0 + 1.0e-10 )  // Check if the point is on the unit sphere
    {
        LOG( FATAL ) << "Point " << query_pt[0] << ", " << query_pt[1] << ", " << query_pt[2]
                     << " is not on the unit sphere";
...
ErrorCode PCDiskAveragedProjectionRemapper::perform_remapping( const ParallelPointCloudReader::PointData& point_data )
{
    ...
    {
        LOG( INFO ) << "Starting PC disk-area averaged projection with " << point_data.size() << " point cloud points";
...
#pragma omp parallel for schedule( dynamic, 8 ) firstprivate( dG, dW ) \
    shared( m_kdtree, element_errors, point_data, m_mesh_data, m_config, m_interface )
...
#pragma omp critical
        ...
        for( int j = 0; j < nP; j++ )
        {
            for( int i = 0; i < nP; i++ )
            {
                int gll_idx = j * nP + i;
                CartVect nodeGLL;
                rval = Remapper_ApplyLocalMap( m_interface, nConnCoords, dG[i], dG[j], nodeGLL );
                if( MB_SUCCESS != rval )
                {
                ...
                bool has_valid_results =
                    !nearest_points.empty() && nearest_points[0].first != static_cast< size_t >( -1 );
    ...
    const ParallelPointCloudReader::PointData& point_data )
{
    ...
    MB_CHK_ERR( m_interface->get_coords( elements.data(), elements.size(), vertex_coords.data() ) );

    Tag areaTag;
    MB_CHK_ERR( m_interface->tag_get_handle( "area", areaTag ) );
    MB_CHK_ERR( m_interface->tag_get_data( areaTag, elements.data(), elements.size(), vertex_areas.data() ) );

    const size_t output_frequency = elements.size() / 10;
#pragma omp parallel for schedule( dynamic, 1 ) \
    shared( m_kdtree, element_errors, point_data, m_mesh_data, m_config, m_interface, vertex_coords, vertex_areas )
...
#pragma omp critical
        ...
        if( elem_idx < 10 )
        {
            std::cout << "Area = " << vertex_areas[elem_idx] << ", Search radius = " << search_radius << std::endl;
        }
