...
{
    // constexpr size_t MAX_ELEMENTS_PER_CHUNK = 500*1000*1000; // 500M elements, well below INT_MAX
    constexpr size_t MAX_ELEMENTS_PER_CHUNK = 100*1000*1000; // 500M elements, well below INT_MAX
    ...
    try
    {
        ...
        if (use_chunked_reading) {
            LOG(INFO) << "Using chunked reading for " << ncol << " points (exceeds " << MAX_ELEMENTS_PER_CHUNK << " limit)";
        }
        ...
        if (use_chunked_reading) {
            ...
            // LOG(INFO) << "Reading data in " << num_chunks << " chunks of up to " << chunk_size << " elements each";

            // Read data chunk by chunk directly into main arrays
            size_t current_start = 0;
            for (size_t chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
                size_t current_count = std::min(chunk_size, static_cast<size_t>(ncol - current_start));

                LOG(INFO) << "Reading chunk " << (chunk_idx + 1) << "/" << num_chunks
                         << " (elements " << current_start << " to " << (current_start + current_count - 1) << ")";

                // Set up NetCDF hyperslab parameters for direct reading into main arrays
                std::vector<MPI_Offset> start = {static_cast<MPI_Offset>(current_start)};
                std::vector<MPI_Offset> read_count = {static_cast<MPI_Offset>(current_count)};

                // Read directly into main arrays at the correct offset - no copying needed!
...
#pragma omp parallel for
                for( size_t idx = current_start; idx < current_start + current_count; ++idx )
                {
                    const size_t offset = (idx - current_start) * 3;
                    RLLtoXYZ_Deg( lon[idx], lat[idx], coords.data() + offset );

                    // Create MOAB vertex entity
                    // MB_CHK_SET_ERR( mb->create_vertex( coords.data() + offset, entities_out[idx] ), "Failed to create vertex" );
                }

                // Create MOAB vertex entity
                Range rngEnts;
                MB_CHK_SET_ERR( mb->create_vertices( coords.data(), current_count, rngEnts ), "Failed to create vertices" );
                std::copy( rngEnts.begin(), rngEnts.end(), entities_out.data() + current_start );
        ...
        Range vertEnts;
        MB_CHK_SET_ERR( mb->get_entities_by_dimension( mesh_set, 0, vertEnts ), "Failed to get vertices" );
        LOG( INFO ) << "Loaded " << vertEnts.size() << " vertices from target NetCDF file " << filename;
...
static ErrorCode write_variable(const PnetCDF::NcmpiVar& var, const ScalarRemapper::MeshData& mesh_data ) {
    ...
    if (var_it != mesh_data.d_scalar_fields.end()) {
        ...
        if (dims.size() == 2)
        {
            ...
            const size_t MAX_ELEMENTS_PER_CHUNK = 250*1000*1000; // 250M elements, well below INT_MAX
            size_t current_start = 0;
            ...
            const size_t MAX_ELEMENTS_PER_CHUNK = 500*1000*1000; // 500M elements, well below INT_MAX
            size_t values_size = values.size();

            if (values_size > MAX_ELEMENTS_PER_CHUNK) {
                LOG(INFO) << "Using chunked writing for 1D variable '" << var.getName() << "' with " << values_size << " elements";

                // Calculate chunk size
                size_t chunk_size = MAX_ELEMENTS_PER_CHUNK;
                size_t num_chunks = (values_size + chunk_size - 1) / chunk_size;

                LOG(INFO) << "Writing 1D variable in " << num_chunks << " chunks of up to " << chunk_size << " elements each";

                // Write data chunk by chunk
                for (size_t chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
                    size_t current_start = chunk_idx * chunk_size;
                    size_t current_count = std::min(chunk_size, values_size - current_start);
                    ...
                    std::vector<MPI_Offset> start = {static_cast<MPI_Offset>(current_start)};
                    std::vector<MPI_Offset> write_count = {static_cast<MPI_Offset>(current_count)};

                    // Convert and write chunk directly
                    std::vector<T> chunk_buffer(current_count);
                    std::transform(values.begin() + current_start, values.begin() + current_start + current_count,
                                  chunk_buffer.begin(), [](double d) { return static_cast<T>(d); });
        ...
        if (ivar_it != mesh_data.i_scalar_fields.end()) {
            ...
            if (dims.size() == 2) {
                ...
                if (values_size > MAX_ELEMENTS_PER_CHUNK) {
                    LOG(INFO) << "Using chunked writing for 1D integer variable '" << var.getName() << "' with " << values_size << " elements";

                    // Calculate chunk size
                    size_t chunk_size = MAX_ELEMENTS_PER_CHUNK;
                    size_t num_chunks = (values_size + chunk_size - 1) / chunk_size;

                    LOG(INFO) << "Writing 1D integer variable in " << num_chunks << " chunks of up to " << chunk_size << " elements each";
    ...
    try
    {
        ...
        auto write_variable = [&]( const std::string& var_name ) -> ErrorCode {
            ...
            if (use_chunked_writing) {
                LOG(INFO) << "Using chunked writing for " << num_entities << " entities (exceeds " << MAX_ELEMENTS_PER_CHUNK << " limit)";

                // Calculate chunk size
                size_t chunk_size = MAX_ELEMENTS_PER_CHUNK;
                size_t num_chunks = (num_entities + chunk_size - 1) / chunk_size;

                LOG(INFO) << "Writing " << var_name << " in " << num_chunks << " chunks of up to " << chunk_size << " elements each";

                // Write data chunk by chunk
                for (size_t chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
                    size_t current_start = chunk_idx * chunk_size;
                    size_t current_count = std::min(chunk_size, num_entities - current_start);

                    LOG(INFO) << "Writing chunk " << (chunk_idx + 1) << "/" << num_chunks
                             << " (entities " << current_start << " to " << (current_start + current_count - 1) << ")";
                ...
                // Write all data at once for smaller datasets
                LOG(INFO) << "Writing all " << num_entities << " " << var_name << " values at once";
