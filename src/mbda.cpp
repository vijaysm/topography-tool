...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    opts.addOpt<std::string>(
        "output", "Output mesh file with remapped data (ending with nc or h5m)",
        &output_file);
    ...
    opts.addOpt<std::string>("fields", "Comma-separated field names to remap",
                             &fields_string_list);
    ...
    opts.addOpt<void>("verbose,v",
                      "Enable verbose output with timestamps. Default: false",
                      &verbose);
    ...
    if (spectral_target) {
      ...
      MB_CHK_SET_ERR(
          mb->load_file(target_file.c_str(), &mesh_set, read_opts.c_str()),
      ...
      if (remapper) {
        ...
        // Serialize remapped results to disk with timing
        {
          ...
          if (write_nc_file) {
            ...
            MB_CHK_SET_ERR(mb->write_file(output_file.c_str(), nullptr, nullptr,
                                          &mesh_set, 1),
          ...
          auto write_time =
              std::chrono::duration_cast<std::chrono::milliseconds>(
                  std::chrono::high_resolution_clock::now() - start_time);
          LOG(INFO) << "Output written in " << write_time.count() / 1000.0
                    << " seconds";
        }
      } else {
