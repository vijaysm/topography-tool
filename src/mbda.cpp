...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        opts.addOpt<std::string>("source", "Source NetCDF point cloud file",
                                 &source_file);
        opts.addOpt<std::string>("target", "Target mesh file (nc or H5M)",
                                 &target_file);
        opts.addOpt<std::string>(
            "output",
            "Output mesh file with remapped data (ending with nc or h5m)",
            &output_file);
        ...
        opts.addOpt<std::string>(
            "lon-var",
            "Longitude variable name (bypasses format detection). Default: lon",
            &lon_variable);
        opts.addOpt<std::string>(
            "lat-var",
            "Latitude variable name (bypasses format detection). Default: lat",
            &lat_variable);
        opts.addOpt<std::string>(
            "area-var", "Area variable name to read and store. Default: area",
            &area_variable);

        opts.addOpt<std::string>("fields",
                                 "Comma-separated field names to remap",
                                 &fields_string_list);
        ...
        opts.addOpt<void>(
            "verbose,v",
            "Enable verbose output with timestamps. Default: false", &verbose);
        ...
        defaultConf.set(el::Level::Global, el::ConfigurationType::ToFile,
                        "false");
        ...
        if (!lon_variable.empty() && !lat_variable.empty()) {
            LOG(INFO) << "Coordinate variable overrides: " << lon_variable
                      << ", " << lat_variable;
        }
        ...
        const bool build_target_from_source =
            reuse_source_mesh && !spectral_target;
        ...
        if (spectral_target) {
            ...
            MB_CHK_SET_ERR(mb->load_file(target_file.c_str(), &mesh_set,
                                         read_opts.c_str()),
            ...
            MB_CHK_SET_ERR(NetcdfMeshIO::load_point_cloud_from_file(
                               mb, mesh_set, target_file, load_opts, entities),
        ...
        if (!square_fields_string_list.empty()) {
            config.square_field_names =
                parse_comma_separated(square_fields_string_list);
        }
        ...
        MB_CHK_SET_ERR(reader.read_points(local_points),
        ...
        if (reader.get_config().print_statistics) {
            ...
            LOG(INFO) << "Total points in dataset: "
                      << reader.get_point_count();
            LOG(INFO) << "Points read: " << local_points.size();
            LOG(INFO) << "Reading time: " << read_duration.count() / 1000.0
                      << " seconds";
        }
        ...
        if (!output_file.empty()) {
            ...
            RemapperFactory::RemapMethod method =
                RemapperFactory::ALG_DISKAVERAGE;
            ...
            auto remapper =
                RemapperFactory::create_remapper(method, mb, mesh_set);
            ...
            if (remapper) {
                ...
                remap_config.scalar_var_names =
                    reader.get_config().scalar_var_names;
                ...
                for (const auto &field_name :
                     reader.get_config().square_field_names) {
                    remap_config.scalar_var_names.push_back(field_name +
                                                            "_squared");
                }
                ...
                MB_CHK_SET_ERR(remapper->configure(remap_config),
                               "Failed to configure remapper");

                // Perform the actual scalar remapping from point cloud to mesh
                MB_CHK_SET_ERR(remapper->remap_scalars(local_points),
                               "Failed to perform scalar remapping");

                // Write remapped data to MOAB tags on mesh elements
                if (!build_target_from_source) {
                    MB_CHK_SET_ERR(remapper->write_to_tags(""),
                                   "Failed to write remapped data to tags");
                }

                LOG(INFO)
                    << "Successfully created remapped tags on mesh elements";
                LOG(INFO) << "";
                ...
                // Serialize remapped results to disk with timing
                {
                    ...
                    if (write_nc_file) {
                        ...
                        write_request.scalar_var_names =
                            reader.get_config().scalar_var_names;
                        write_request.squared_var_names =
                            reader.get_config().square_field_names;
                        ...
                        if (!build_target_from_source) {
                            ...
                            MB_CHK_SET_ERR(
                                NetcdfMeshIO::write_point_scalars_to_file(
                                    mb, target_file, output_file, write_request,
                                    entities),
                                "Failed to write NetCDF output");
                        } else {
                            // Write from MeshData (source-based target)
                            const auto &get_mesh_data =
                                remapper->get_mesh_data();
                            MB_CHK_SET_ERR(
                                NetcdfMeshIO::write_point_scalars_to_file(
                                    mb, source_file, output_file, write_request,
                                    get_mesh_data),
                                "Failed to write NetCDF output");
                        }
                    } else {
                        // Write H5M output format (mesh with tags)
                        MB_CHK_SET_ERR(mb->write_file(output_file.c_str(),
                                                      nullptr, nullptr,
                                                      &mesh_set, 1),
                    ...
                    LOG(INFO)
                        << "Saved mesh with remapped data to: " << output_file;
                    ...
                    auto write_time =
                        std::chrono::duration_cast<std::chrono::milliseconds>(
                            ...
                            start_time);
                    LOG(INFO) << "Output written in "
                              << write_time.count() / 1000.0 << " seconds";
                }
            } else {
